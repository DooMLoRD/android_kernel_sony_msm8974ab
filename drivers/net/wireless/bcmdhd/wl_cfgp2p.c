 ...
 *
 * $Id: wl_cfgp2p.c 449134 2014-01-16 07:59:17Z $
 *
 */
#include <typedefs.h>
#include <linuxver.h>
...
/* Return -1 or the index where the token has been found. */
static int wl_cfgp2p_is_p2p_gas_dialog_token(struct p2p_info *p2p, int dialog_token)
{
	int ret = -1;
	int i = 0;
	while ((i < P2P_MAX_PENDING_GAS) && (p2p->gas_dialog_token[i] != dialog_token)) {
		i++;
	}
	if (i < P2P_MAX_PENDING_GAS) {
		ret = i;
	}
	return ret;
}

static void wl_cfgp2p_gas_clear_dialog_token_index(struct p2p_info *p2p, int index)
{
	if (p2p && (index >= 0) && (index < P2P_MAX_PENDING_GAS)) {
		p2p->gas_dialog_token[index] = P2P_GAS_NOT_A_DIALOG_TOKEN;
	}
}

bool wl_cfgp2p_is_p2p_gas_action_resp(struct wl_priv *wl, void *frame, u32 frame_len)
{

	wifi_p2psd_gas_pub_act_frame_t *sd_act_frm;
	int index = -1;

	if (frame == NULL || wl == NULL)
		return false;
	...
	/* Use the dialog token to identify a response to a P2P GAS request. */
	if ((sd_act_frm->action == P2PSD_ACTION_ID_GAS_IRESP ||
		sd_act_frm->action == P2PSD_ACTION_ID_GAS_CRESP) &&
		(wl->p2p) &&
		((index = wl_cfgp2p_is_p2p_gas_dialog_token(wl->p2p,
		sd_act_frm->dialog_token)) >= 0)) {
		wl_cfgp2p_gas_clear_dialog_token_index(wl->p2p, index);
		return true;
	} else {
		return false;
	}
...
/* Record GAS dialog token in first available slot. */
int wl_cfgp2p_gas_rec_dialog_token(struct wl_priv *wl, int dialog_token)
{
	int i = 0;
	int ret = -1;
	if (wl->p2p) {
		while (i < P2P_MAX_PENDING_GAS && wl->p2p->gas_dialog_token[i] !=
			P2P_GAS_NOT_A_DIALOG_TOKEN) {
			i++;
		}
		if (i < P2P_MAX_PENDING_GAS) {
			wl->p2p->gas_dialog_token[i] = dialog_token;
			ret = i;
		}
	}
	return ret;
...
#ifdef WL11U
	for (i = 0; i < P2P_MAX_PENDING_GAS; i++) {
		wl->p2p->gas_dialog_token[i] = P2P_GAS_NOT_A_DIALOG_TOKEN;
	}
#endif /* WL11U */
	return BCME_OK;
