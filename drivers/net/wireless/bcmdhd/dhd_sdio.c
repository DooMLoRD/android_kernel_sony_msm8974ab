...
int
somc_txpower_get_min_delta(void)
{
	return (somc_txpower_min_delta == INT_MAX) ? 0 : somc_txpower_min_delta;
}

static void
somc_txpower_update_min_delta(const int *delta, int num)
{
	while (num > 0) {
		num--;
		somc_txpower_min_delta = MIN(somc_txpower_min_delta, delta[num]);
	}
}
...
{
	const unsigned char *end = nvram + nvram_len - 1;
	unsigned char *k, *v, *t;
	unsigned long power;
	int i, j;
	...
	v[len] = 0;
	if (kstrtoul(v, 16, &power)) {
		v[len] = '\n';
		DHD_ERROR(("%s: failed to convert to a hex digits: %s\n", __FUNCTION__, key));
		return -1;
	}
	...
	for (i = 0, j = len - offset; i < j; i++) {
		int val, sft = i * 4;
		val = ((power >> sft) & 0xf) - delta;
		val = (val > 0xf) ? 0xf : (val < 0x0) ? 0x0 : val;
		power &= ~(0xf << sft);
		power |= val << sft;
	}

	if (len == 4) {
		snprintf(v, len + 1, "%04x", (unsigned int)power);
	} else if (len == 8) {
		snprintf(v, len + 1, "%08x", (unsigned int)power);
	}
	v[len] = '\n';
...
static int
somc_txpower_calibrate(char *nvram, int nvram_len)
{
	int delta[SOMC_TXPWR_BUF_SIZE] = {0}; /* tx power offset(s) */

	/* initialize minimum delta (used for tx power back-off) */
	somc_txpower_min_delta = INT_MAX;

	/* 2.4GHz range */
	if (somc_read_ta(SOMC_TA_TXPWR_2_4G, (char *)delta, sizeof(delta)) == 0) {
		if (somc_txpower_apply_delta(SOMC_CKEY_TXPWR_2_4G_11B,
					     4, 0, delta[0], nvram, nvram_len) != 0)
			goto fail;
		if (somc_txpower_apply_delta(SOMC_CKEY_TXPWR_2_4G_11A,
					     4, 0, delta[1], nvram, nvram_len) != 0)
			goto fail;
		if (somc_txpower_apply_delta(SOMC_CKEY_TXPWR_2_4G_11A_2,
					     4, 2, delta[1], nvram, nvram_len) != 0)
			goto fail;
		if (somc_txpower_apply_delta(SOMC_CKEY_TXPWR_2_4G_11N,
					     8, 0, delta[2], nvram, nvram_len) != 0)
			goto fail;
		somc_txpower_update_min_delta(delta, SOMC_TXPWR_BUF_SIZE);
	}

	/* 5.2GHz/5.3GHz range */
	if (somc_read_ta(SOMC_TA_TXPWR_5G_LOW, (char *)delta, sizeof(delta)) == 0) {
		if (somc_txpower_apply_delta(SOMC_CKEY_TXPWR_5G_LOW_11A,
					     8, 0, delta[1], nvram, nvram_len) != 0)
			goto fail;
		if (somc_txpower_apply_delta(SOMC_CKEY_TXPWR_5G_LOW_11N,
					     8, 0, delta[3], nvram, nvram_len) != 0)
			goto fail;
		if (somc_txpower_apply_delta(SOMC_CKEY_TXPWR_5G_LOW_11AC,
					     8, 0, delta[6], nvram, nvram_len) != 0)
			goto fail;
		somc_txpower_update_min_delta(delta, SOMC_TXPWR_BUF_SIZE);
	}

	/* 5.6GHz range */
	if (somc_read_ta(SOMC_TA_TXPWR_5G_MID, (char *)delta, sizeof(delta)) == 0) {
		if (somc_txpower_apply_delta(SOMC_CKEY_TXPWR_5G_MID_11A,
					     8, 0, delta[1], nvram, nvram_len) != 0)
			goto fail;
		if (somc_txpower_apply_delta(SOMC_CKEY_TXPWR_5G_MID_11N,
					     8, 0, delta[3], nvram, nvram_len) != 0)
			goto fail;
		if (somc_txpower_apply_delta(SOMC_CKEY_TXPWR_5G_MID_11AC,
					     8, 0, delta[6], nvram, nvram_len) != 0)
			goto fail;
		somc_txpower_update_min_delta(delta, SOMC_TXPWR_BUF_SIZE);
	}
	...
	/* 5.8GHz range */
	if (somc_read_ta(SOMC_TA_TXPWR_5G_HIGH, (char *)delta, sizeof(delta)) == 0) {
		if (somc_txpower_apply_delta(SOMC_CKEY_TXPWR_5G_HIGH_11A,
					     8, 0, delta[1], nvram, nvram_len) != 0)
			goto fail;
		if (somc_txpower_apply_delta(SOMC_CKEY_TXPWR_5G_HIGH_11N,
					     8, 0, delta[3], nvram, nvram_len) != 0)
			goto fail;
		if (somc_txpower_apply_delta(SOMC_CKEY_TXPWR_5G_HIGH_11AC,
					     8, 0, delta[6], nvram, nvram_len) != 0)
			goto fail;
		somc_txpower_update_min_delta(delta, SOMC_TXPWR_BUF_SIZE);
	}
