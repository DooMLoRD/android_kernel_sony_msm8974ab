...
#ifdef SOMC_MIRACAST_STATISTICS
#define MAX_WLIW_IOCTL_LEN	1024
#define CMD_SOMC_GET_MIRACAST_STATS	"SOMC_GET_MIRACAST_STATS"

struct miracast_priv_stats {
	u32 txframe;
	u32 txbyte;
	u32 txerror;
	u32 rxframe;
...
static int
wl_android_somc_get_miracast_stats(struct net_device *dev, char *command, int total_len)
{
	wl_cnt_t cnt;
	int link_speed = 0;
	struct miracast_priv_stats *output;
	unsigned int bufsize = 0;
	int bytes_written = 0;
	int ret = 0;

	DHD_INFO(("%s: enter SOMC miracast stats\n", __FUNCTION__));

	if (total_len <= 0) {
		DHD_ERROR(("%s: invalid buffer size %d\n", __FUNCTION__, total_len));
		goto error;
	}

	bufsize = total_len;
	if (bufsize < sizeof(struct miracast_priv_stats)) {
		DHD_ERROR(("%s: not enough buffer size, provided=%u, requires=%u\n",
			__FUNCTION__, bufsize,
			sizeof(struct miracast_priv_stats)));
		goto error;
	}

	memset(&cnt, 0, sizeof(wl_cnt_t));
	ret = dev_wlc_bufvar_get(dev, "counters", (char *)&cnt, sizeof(wl_cnt_t));
	if (ret) {
		DHD_ERROR(("%s: dev_wlc_bufvar_get() failed, ret=%d\n",
			__FUNCTION__, ret));
		goto error;
	}

	/*
	 * Note: currently WL_CNT_T_VERSION is defined as 8, but the driver returns
	 *       9 for cnt.version. I hope we can still use version 8 of wl_cnt_t
	 *       structure for the ioctl.
	 */
	if (dtoh16(cnt.version) != 9 /* WL_CNT_T_VERSION */) {
		DHD_ERROR(("%s: incorrect version of wl_cnt_t, expected=%u got=%u\n",
			__FUNCTION__, 9 /* WL_CNT_T_VERSION */, cnt.version));
		goto error;
	}
	...
	if (ret || link_speed < 0) {
		DHD_ERROR(("%s: wldev_get_link_speed() failed, ret=%d, speed=%d\n",
			__FUNCTION__, ret, link_speed));
		goto error;
	}

	output = (struct miracast_priv_stats *)command;
	...
	bytes_written = sizeof(struct miracast_priv_stats);
	return bytes_written;
...
#define PRIVATE_COMMAND_MAX_LEN	8192
	...
	/* CUSTOMER_SET_COUNTRY feature is define for only GGSM model */
		...
		bytes_written = wl_android_set_ibss_beacon_ouidata(net, command,
			priv_cmd.total_len);
#ifdef SOMC_MIRACAST_STATISTICS
	else if (strnicmp(command, CMD_SOMC_GET_MIRACAST_STATS,
		strlen(CMD_SOMC_GET_MIRACAST_STATS)) == 0)
		bytes_written = wl_android_somc_get_miracast_stats(net, command,
			priv_cmd.total_len);
#endif
	else {
