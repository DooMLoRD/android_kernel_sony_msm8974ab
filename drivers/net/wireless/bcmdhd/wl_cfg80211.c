...
static const struct ieee80211_regdomain brcm_regdom = {
	...
	(defined(WL_IFACE_COMB_NUM_CHANNELS) || 0)
/*
...
#if defined(WL11U)
	/* Record the dialog token in case of GAS request for P2P. */
	if ((action == P2PSD_ACTION_ID_GAS_IREQ || action == P2PSD_ACTION_ID_GAS_CREQ) &&
		(ndev_to_wdev(ndev) != wl_to_prmry_wdev(wl)) && (wl->p2p)) {
		wifi_p2psd_gas_pub_act_frame_t *sd_act_frm;
		int index = 0;
		sd_act_frm = (wifi_p2psd_gas_pub_act_frame_t *)action_frame->data;
		index = wl_cfgp2p_gas_rec_dialog_token(wl, sd_act_frm->dialog_token);
		if (index < 0) {
			WL_ERR(("P2P: GAS: Couldn't record token %d!\n", sd_act_frm->dialog_token));
		}
	}
#endif /* WL11U */
...
#if defined(WL11U)
		(is_p2p_gas_resp = wl_cfgp2p_is_p2p_gas_action_resp(wl,
			&mgmt_frame[DOT11_MGMT_HDR_LEN], mgmt_frame_len - DOT11_MGMT_HDR_LEN)) ||
#endif /* WL11U */
		wl_cfgp2p_is_gas_action(&mgmt_frame[DOT11_MGMT_HDR_LEN],
			mgmt_frame_len - DOT11_MGMT_HDR_LEN) ||
		0) {
