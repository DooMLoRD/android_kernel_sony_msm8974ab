...
extern int somc_txpower_get_min_delta(void);
...
static int somc_update_qtxpower(char *buf, int len)
{
	int in_qdbm, power;
	int delta = somc_txpower_get_min_delta();

	if (len < 4)
		return -1;

	in_qdbm = dtoh32(*(int *)buf);
	...
	DHD_INFO(("%s: Set max tx power: %d qdBm (delta=%d)\n",
		  __FUNCTION__, dtoh32(power), delta));
...
int dhd_ioctl_process(dhd_pub_t *pub, int ifidx, dhd_ioctl_t *ioc)
{
	...
	if (ioc->cmd == WLC_SET_VAR && ioc->buf != NULL &&
	    strncmp("qtxpower", ioc->buf, 8) == 0) {
		if (somc_update_qtxpower((char *)buf + 9, buflen - 9) != 0) {
			bcmerror = BCME_ERROR;
			goto done;
		}
	}
